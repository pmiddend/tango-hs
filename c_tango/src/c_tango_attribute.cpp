...
static void convert_attribute_reading(Tango::DeviceAttribute &devattr,
                                      AttributeData *argout);
static void convert_attribute_writing(AttributeData *argin,
                                      Tango::DeviceAttribute &devattr);
static void convert_attr_query(Tango::AttributeInfoEx &tango_attr_info,
...
ErrorStack *tango_read_attributes(void *proxy, VarStringArray *attr_names,
  ...
  std::vector<Tango::DeviceAttribute> *devattr_list = 0;
...
ErrorStack *tango_write_attributes(void *proxy, AttributeDataList *argin) {
  Tango::DeviceProxy *dev = (Tango::DeviceProxy *)proxy;
  std::vector<Tango::DeviceAttribute> devattr_list(argin->length);
...
#define DELETE_SEQ(member)                                                     \
  ...
  case DEV_UCHAR:
    DELETE_SEQ(char_arr);
  case DEV_SHORT:
    DELETE_SEQ(short_arr);
  case DEV_USHORT:
    DELETE_SEQ(ushort_arr);
  case DEV_LONG:
    DELETE_SEQ(long_arr);
  case DEV_ULONG:
    DELETE_SEQ(ulong_arr);
  case DEV_LONG64:
    DELETE_SEQ(long64_arr);
  case DEV_ULONG64:
    DELETE_SEQ(ulong64_arr);
  case DEV_FLOAT:
    DELETE_SEQ(float_arr);
  case DEV_DOUBLE:
    DELETE_SEQ(double_arr);
  case DEV_STATE:
    DELETE_SEQ(state_arr);
  ...
  case DEV_ENCODED:
    for (uint32_t i = 0; i < attribute_data->attr_data.encoded_arr.length;
         i++) {
      free(attribute_data->attr_data.encoded_arr.sequence[i]
               .encoded_format); // from strdup
...
ErrorStack *tango_get_attribute_list(void *proxy, VarStringArray *attr_names) {
  Tango::DeviceProxy *dev = (Tango::DeviceProxy *)proxy;
  std::vector<std::string> *attr_list = 0;
...
ErrorStack *tango_get_attribute_config(void *proxy, VarStringArray *attr_names,
  ...
  try {
    ...
    std::cerr << "looping over " << tango_attr_info_list->size()
              << " attributes" << std::endl;
    // loop over all returned attributes and convert the data
    for (uint32_t i = 0; i < tango_attr_info_list->size(); i++) {
      convert_attr_query((*tango_attr_info_list)[i],
                         &attr_info_list->sequence[i]);
    }
...
void tango_free_AttributeInfoList(AttributeInfoList *attribute_info_list) {
  for (uint32_t i = 0; i < attribute_info_list->length; i++) {
    ...
    for (uint16_t j = 0; j < attribute_info_list->sequence[i].enum_labels_count;
         ++j)
...
void convert_attribute_reading(Tango::DeviceAttribute &devattr,
                               AttributeData *argout) {
...
#define EXTRACT_ARRAY(member, tgtype, type)                                    \
  {                                                                            \
    ...
    case DEV_BOOLEAN:
      EXTRACT_ARRAY(bool_arr, DevVarBooleanArray, bool);
    case DEV_UCHAR:
      EXTRACT_ARRAY(char_arr, DevVarCharArray, uint8_t);
    case DEV_SHORT:
      EXTRACT_ARRAY(short_arr, DevVarShortArray, int16_t);
    case DEV_ENUM:
      EXTRACT_ARRAY(short_arr, DevVarShortArray, int16_t);
    case DEV_USHORT:
      EXTRACT_ARRAY(ushort_arr, DevVarUShortArray, uint16_t);
    case DEV_LONG:
      EXTRACT_ARRAY(long_arr, DevVarLongArray, Tango::DevLong);
    case DEV_ULONG:
      EXTRACT_ARRAY(ulong_arr, DevVarULongArray, Tango::DevULong);
    case DEV_LONG64:
      EXTRACT_ARRAY(long64_arr, DevVarLong64Array, Tango::DevLong64);
    case DEV_ULONG64:
      EXTRACT_ARRAY(ulong64_arr, DevVarULong64Array, Tango::DevULong64);
    case DEV_FLOAT:
      EXTRACT_ARRAY(float_arr, DevVarFloatArray, float);
    case DEV_DOUBLE:
      EXTRACT_ARRAY(double_arr, DevVarDoubleArray, double);
    ...
    case DEV_STRING: {
      ...
      for (int i = 0; i < nb_data; i++) {
        argout->attr_data.string_arr.sequence[i] =
    ...
    case DEV_STATE: {
      std::vector<Tango::DevState> state_vect;
...
void convert_attribute_writing(AttributeData *argin,
                               Tango::DeviceAttribute &devattr) {
  // allocate a vector and copy the data

#define INSERT_ARRAY(member, type)                                             \
  {                                                                            \
    std::vector<type> arr(argin->attr_data.member.length);                     \
  ...
  case DEV_BOOLEAN: {
    std::vector<bool> arr(argin->attr_data.bool_arr.length);
  ...
  case DEV_UCHAR:
    INSERT_ARRAY(char_arr, uint8_t);
  case DEV_SHORT:
    INSERT_ARRAY(short_arr, int16_t);
  case DEV_USHORT:
    INSERT_ARRAY(ushort_arr, uint16_t);
  case DEV_LONG:
    INSERT_ARRAY(long_arr, Tango::DevLong);
  case DEV_ULONG:
    INSERT_ARRAY(ulong_arr, Tango::DevULong);
  case DEV_LONG64:
    INSERT_ARRAY(long64_arr, Tango::DevLong64);
  case DEV_ULONG64:
    INSERT_ARRAY(ulong64_arr, Tango::DevULong64);
  case DEV_FLOAT:
    INSERT_ARRAY(float_arr, float);
  case DEV_DOUBLE:
    INSERT_ARRAY(double_arr, double);

  case DEV_STRING: {
    std::vector<std::string> string_arr(argin->attr_data.string_arr.length);
  ...
  case DEV_STATE: {
    std::vector<Tango::DevState> state_arr(argin->attr_data.state_arr.length);
...
static void convert_attr_query(Tango::AttributeInfoEx &tango_attr_info,
  ...
  attr_info->writable_attr_name =
      strdup(tango_attr_info.writable_attr_name.c_str());
