...
public:
  haskellAttrib(AttributeDefinitionCpp const &def)
      : Attr(def.name.c_str(), static_cast<long>(def.data_type),
             Tango::OPERATOR,
             static_cast<Tango::AttrWriteType>(def.write_type)),
        def{def}, last_ptr{0}, last_container{0} {}
...
JustOneAttributeClass::JustOneAttributeClass(std::string &s)
    : Tango::DeviceClass(s) {
...
void JustOneAttributeClass::attribute_factory(
    std::vector<Tango::Attr *> &att_list) {
  for (AttributeDefinitionCpp const &attribute_definition :
...
void JustOneAttributeClass::device_factory(
  ...
  for (unsigned long i = 1; i <= devlist_ptr->length(); i++) {
    ...
    JustOneAttribute *dev =
        static_cast<JustOneAttribute *>(device_list[device_list.size() - i]);
...
void JustOneAttributeClass::create_static_attribute_list(
    std::vector<Tango::Attr *> &att_list) {
...
void JustOneAttribute::delete_device() {
  DEBUG_STREAM << "JustOneAttribute::delete_device() " << device_name
               << std::endl;
  // delete[] attr_philipp_read;
}

void JustOneAttribute::init_device() {
  DEBUG_STREAM << "JustOneAttribute::init_device() create device "
...
void JustOneAttribute::read_attr_hardware(
...
void JustOneAttribute::write_attr_hardware(
    TANGO_UNUSED(std::vector<long> &attr_list)) {
...
void tango_server_set_state(int const new_state) {
  JustOneAttribute::instance()->set_state(
      static_cast<Tango::DevState>(new_state));
}
