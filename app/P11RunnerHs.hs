...
import UnliftIO.Foreign (peekCString, withArray, withCString)
...
tangoReadProperty :: (UnliftIO.MonadUnliftIO m) => InitedServer -> PropertyName -> m Text
tangoReadProperty _initedServer (PropertyName n) = do
  resultAsCString <- withCStringFromText n (\x -> liftIO (tango_server_read_property x))
